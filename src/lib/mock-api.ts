...
const TRADES = ["Plumber", "Electrician", "Carpenter", "Painter", "HVAC Technician", "Mason", "Roofer", "Landscaper"];
const LOCATIONS = ["New York", "London", "San Francisco", "Berlin", "Paris", "Tokyo", "Sydney", "Toronto"];
...
const MOCK_ARTISANS: Artisan[] = Array.from({ length: 40 }).map((_, i) => ({
  id: `artisan-${i + 1}`,
  name: REAL_NAMES[i],
  trade: TRADES[i % TRADES.length],
  rating: 3.5 + (i % 15) / 10, // 3.5 to 4.9
  ...
  bio: `${REAL_NAMES[i]} is a highly skilled ${TRADES[i % TRADES.length]} with extensive experience in residential and commercial projects. Known for reliability and attention to detail.`,
  portfolio: [
